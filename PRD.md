...
**Status:** Draft v1 — covers MVP scope per planning Rounds 1–7.
...
Every FR has: **ID** · **Priority** (P0 = MVP, P1 = post-MVP) · **Personas affected** · **Description** · **User stories** · **Acceptance criteria**. Cross-references to PLAN.md sections in parens.

### FR-1 — Workspaces & multi-tenancy (PLAN §6)
- **Priority:** P0 · **Personas:** P-1, P-3
...
- **User stories:**
  - As a manager, I can create a new workspace and invite up to 19 teammates.
  - As an admin, I can change a teammate's role.
  - As a rep, I can switch between workspaces I belong to without logging out.
- **Acceptance criteria:**
...
- **Priority:** P0 · **Personas:** P-1, P-2, P-3, P-4
...
- **User stories:**
  ...
  - As a returning user, I can log in on web and iOS with the same credentials and stay logged in.
...
- **Priority:** P0 · **Personas:** P-2, P-3
- **Description:** First/last name, primary email, phone(s), title, company FK, owner FK, lifecycle stage, source, custom fields.
- **User stories:**
  - As a rep, I can create, view, edit, and archive a contact.
  - As a rep, I can search contacts by name, email, or any custom field.
  - As an admin, I can define custom fields per workspace.
- **Acceptance criteria:**
  - List view paginates 1k contacts smoothly (≤200ms after filter change).
  - Detail view shows the full activity timeline.
...
- **User stories:**
  ...
  - As a rep, I can navigate to a deal directly from a company page.
- **Acceptance criteria:**
  - Domain auto-uniqueness check on create.
  - Company merge (post-MVP) flagged in P1.

### FR-5 — Deals & pipelines (PLAN §6)
- **Priority:** P0 · **Personas:** P-1, P-2
...
- **User stories:**
  - As a manager, I can create a pipeline and order its stages.
  - As a rep, I can drag a deal across stages on a kanban view.
  - As a manager, I can switch a pipeline view to a list view.
- **Acceptance criteria:**
  - Stage change emits an `activity` row + a `record_field_changed` automation event.
  - Currency stored in cents; display localized.

### FR-6 — Tasks (PLAN §6)
- **Priority:** P0 · **Personas:** P-1, P-2
- **Description:** Title, due, owner, polymorphic relation (contact/company/deal), status, priority, custom fields.
- **User stories:**
  - As a rep, I can create a task on any record from its detail view.
  - As a rep, I see today's tasks in a "today" smart view.
- **Acceptance criteria:**
  - Overdue tasks highlight in tan-text accent.
  - Tasks support `assigned_to_workspace_id` for cross-workspace? No — tasks scoped per workspace.

### FR-7 — Notes & activities (PLAN §6)
- **Priority:** P0 · **Personas:** P-2
...
- **User stories:**
  - As a rep, I can add a note to a contact and edit it within 24 hours.
  - As a rep, I see a chronological activity feed on every record.
- **Acceptance criteria:**
  - Activity feed paginates infinite scroll.
  - Note edits beyond 24h create an audit trail.

### FR-8 — Custom fields (PLAN §6)
- **Priority:** P0 · **Personas:** P-3
...
- **User stories:**
  ...
  - As a rep, I can filter and sort by custom fields.
- **Acceptance criteria:**
  - Adding a field doesn't require a schema migration (JSONB-backed).
  - Hot fields can be promoted to indexed expression columns by an admin (post-MVP shortcut: a single index_now flag).

### FR-9 — Email sync (Gmail) (PLAN §4.4)
- **Priority:** P0 · **Personas:** P-2
...
- **User stories:**
  - As a rep, I can connect my Gmail and see threads with each contact.
  - As a rep, I can send an email to a contact from inside the contact view.
  - As a rep, I receive in-app activity when a contact replies.
- **Acceptance criteria:**
  - Initial sync ingests last 90 days; ongoing sync is real-time via Pub/Sub push.
  - Disconnect revokes tokens server-side.

### FR-10 — Calendar sync (Google Calendar) (PLAN §4.4)
- **Priority:** P0 · **Personas:** P-1, P-2
- **Description:** OAuth-connect Google Calendar. Sync events to record activity timelines and trigger automations.
- **User stories:**
  - As a rep, I see my upcoming meetings on each contact/deal record.
  - As a rep, I can create a calendar event from a contact (auto-invite the contact).
- **Acceptance criteria:**
  - Past meeting summaries (FR-13) appear within 30s of the meeting ending.

### FR-11 — Workflow automation engine (PLAN §7)
- **Priority:** P0 · **Personas:** P-1, P-3
...
- **User stories:**
  - As an admin, I can build a workflow that triggers on "deal moved to Negotiation" and waits 1 day before drafting an AI follow-up email.
  - As a rep, I see workflow runs in a per-record activity feed.
  - As an admin, I can replay or cancel a stuck run.
- **Acceptance criteria:**
  - Worker restart mid-run does not lose state (idempotency + persisted state_snapshot).
  - HITL approval token signed and single-use; expires in 7 days.
  - Run inspector shows step-by-step input/output/cost/latency.

### FR-12 — Workflow node-graph editor (PLAN §7.2)
- **Priority:** P0 · **Personas:** P-1, P-3
...
- **User stories:**
  - As an admin, I can drag nodes onto the canvas and connect them with edges.
  - As an admin, I can save a draft and publish it as a new version.
  - As an admin, I can describe a workflow in English and let Claude generate the node graph.
- **Acceptance criteria:**
  - Canvas renders 100-node workflows without jank.
  - Undo/redo for canvas edits.
  - Schema validation prevents publishing a malformed graph.

### FR-13 — AI: comms intelligence (PLAN §8.4)
- **Priority:** P0 · **Personas:** P-2
- **Description:** Streaming AI drafts, reply suggestions, thread summaries, meeting summaries. Surfaced in every contact + deal view.
- **User stories:**
  - As a rep, I can ask AI to draft a follow-up email for this contact based on our last 3 messages + deal stage.
  - As a rep, I can read a 3-bullet summary of a long Gmail thread.
  - As a rep, after a Google Calendar meeting, I can paste the transcript and get a 5-bullet summary written to the deal's activity log.
- **Acceptance criteria:**
  - First token of a draft arrives ≤1.5s P95.
  - Drafts are editable in-place before send.
  - Cost shown per generation in admin telemetry.

### FR-14 — AI: NL data layer (PLAN §8.2)
- **Priority:** P0 · **Personas:** P-1, P-2
- **Description:** Natural-language queries → structured filter DSL → server-side query → results + AI summary.
- **User stories:**
  - As a manager, I can type "deals over $10k that went quiet last week" into cmd-K and see a filtered list.
  - As a rep, I can save the resulting filter as a saved view.
- **Acceptance criteria:**
  - NL → filter DSL conversion ≤2s P95.
  - Hybrid search (FTS + pgvector rerank) for fuzzy queries.
  - Schema-aware prompting: includes custom fields.

### FR-15 — AI: lead scoring + enrichment (PLAN §8.3)
- **Priority:** P0 · **Personas:** P-2, P-3
- **Description:** Background workers populate lead score + company enrichment fields on new contacts/companies.
- **User stories:**
  - As a rep, when I add a contact with email `john@stripe.com`, I see Stripe's company size and industry auto-fill within 60s.
  - As an admin, I can disable enrichment per workspace.
- **Acceptance criteria:**
  - Per-workspace daily token budget enforced; over-budget → enqueue + notify owner.
  - Manual "re-score" button on each contact.

### FR-16 — AI: suggested next actions (PLAN §8)
- **Priority:** P0 · **Personas:** P-2
- **Description:** Per-record AI suggestion strip. Calm, non-chatbot UX.
- **User stories:**
  - As a rep, on a quiet deal I see "This deal hasn't moved in 5 days — draft a re-engage email?".
  - As a rep, I can dismiss a suggestion or convert it into a task or workflow.
- **Acceptance criteria:**
  - Max 3 suggestions visible at once; rest in an overflow.
  - Dismissed suggestions do not reappear for 7 days.

### FR-17 — AI: agent autonomy + HITL (PLAN §8.5)
- **Priority:** P0 · **Personas:** P-1, P-3
...
- **User stories:**
  - As an admin, I can set "send email to lead" to require approval but "post Slack notification" to be autonomous.
  - As a rep, I can toggle "Agent assist" on a specific deal.
  - As a rep, I receive an iOS push when an agent action needs my approval; I can approve/reject without opening the app fully.
- **Acceptance criteria:**
  - Reversible actions (notes, drafts) get a 60-second undo affordance after autonomous execution.
  - Irreversible actions (sent emails, charged payments — post-MVP) always require approval.
  - Audit log records prompt, model, cost, output, decision, decider.

### FR-18 — Reports & dashboards (PLAN §13 M14)
- **Priority:** P0 (light) · **Personas:** P-1, P-4
- **Description:** Saved-view counters + simple deal funnel + activity-over-time chart.
- **User stories:**
  - As a manager, on workspace home I see this week's pipeline + the team's activity.
  - As a viewer, I can drill from a counter into the underlying records.
- **Acceptance criteria:**
  - Workspace home loads ≤500ms P95 with cached aggregates.
  - Charts respect WCAG 2.2 AA (color is not the sole signal).

### FR-19 — Search (FTS + pgvector) (PLAN §8.2, §13 M12)
- **Priority:** P0 · **Personas:** P-2
- **Description:** Global cmd-K with FTS + AI-summarized results. Hybrid keyword + vector for fuzzy queries.
- **User stories:**
  - As a rep, I can search across contacts, companies, deals, notes, emails from one input.
  - As a rep, my recent searches are remembered.
- **Acceptance criteria:**
  - Cmd-K opens in ≤80ms.
  - First results render ≤200ms after 3 chars.

### FR-20 — Import (CSV, HubSpot, Pipedrive) (PLAN §4.4)
- **Priority:** P0 · **Personas:** P-3
- **Description:** Switching-cost killer: bring your data in 5 minutes.
- **User stories:**
  - As an admin, I can paste a HubSpot export URL and have contacts/companies/deals mapped automatically.
  - As an admin, I can review and adjust the mapping before committing.
  - As an admin, I can import a CSV with up to 50k rows.
- **Acceptance criteria:**
  - Mapping suggestions powered by AI; user-correctable.
  - Import runs as a workflow with progress + error report.

### FR-21 — Slack integration (PLAN §4.4)
- **Priority:** P0 · **Personas:** P-1, P-3
...
- **User stories:**
  - As an admin, I connect Slack and choose channels for different notification types.
  - As a rep, I see workflow output in #sales when configured.
- **Acceptance criteria:**
  - Disconnect revokes tokens; existing workflow steps that point at Slack pause until reconnected.

### FR-22 — Generic webhooks + HTTP requests (PLAN §4.4)
- **Priority:** P0 · **Personas:** P-3
- **Description:** Inbound webhook endpoints (per-workspace HMAC) and outbound HTTP step for workflows.
- **User stories:**
  - As an admin, I can copy a unique webhook URL and POST data to it from any system.
  - As an admin, I can add an HTTP request step in a workflow with custom headers + auth.
- **Acceptance criteria:**
  - Webhook payloads logged for 7 days for debugging.
  - HTTP step supports basic + bearer + custom-header auth presets.

### FR-23 — Native iOS app (PLAN §4.7)
- **Priority:** P0 · **Personas:** P-1, P-2
- **Description:** SwiftUI app with full read+edit on contacts/deals, push notifications, HITL approval surface.
- **User stories:**
  - As a rep, I can sign in on iPhone, browse my pipeline, and edit a deal.
  - As a manager, I receive an APNs push when an agent action needs approval; I can approve/reject from the lock screen action.
- **Acceptance criteria:**
  - App launches to "today" view ≤2s on a 3-year-old iPhone.
  - Offline read for last 50 visited records; edits queue and sync on reconnect.
  - App Store review checklist locally green.

### FR-24 — Notifications (in-app, email, iOS push) (PLAN §4.6)
- **Priority:** P0 · **Personas:** P-1, P-2, P-3
- **Description:** In-app inbox + email (Resend) + iOS push (APNs). User-controlled per-channel preferences.
- **User stories:**
  - As a rep, I can disable email for `task_due_soon` while keeping push on.
  - As an admin, I can see which notifications were sent in the last 7 days.
- **Acceptance criteria:**
  - Quiet hours per user (no push between 9pm–7am local by default).
  - HITL approval pushes always bypass quiet hours.

### FR-25 — Audit logs (light) (PLAN §8.5)
- **Priority:** P0 · **Personas:** P-3
- **Description:** Append-only `activity` rows already cover most actions; admin UI surfaces a filterable log.
- **User stories:**
  - As an admin, I can see who changed which deal stage in the last 30 days.
  - As an admin, I can export an audit slice as CSV.
- **Acceptance criteria:**
  - 90-day retention in MVP; longer retention is post-MVP enterprise.
...
### NFR-1 — Performance
- Cmd-K open ≤80ms; first results ≤200ms after 3 chars.
- Page-to-page nav P95 ≤300ms (RSC + cached data).
- AI streaming: first token P95 ≤1.5s.
- iOS app cold start P95 ≤2s on 3-year-old hardware.
- Workflow advancer step latency P95 ≤500ms (excluding intentional waits/AI).

### NFR-2 — Accessibility (WCAG 2.2 AA)
- Keyboard navigation across every flow (cmd-K, j/k, slash, esc).
...
- Axe-clean on all top-level routes; manual screen-reader pass before each release.

### NFR-3 — Security
- All endpoints require JWT except `/auth/*` and `/health`.
- Tenant isolation enforced at the ORM layer (workspace mixin) + verified by tests.
- Secrets never logged; Sentry scrubs PII.
- All OAuth tokens encrypted at rest with a workspace-scoped key.
- HMAC on inbound webhooks; signed approval tokens for HITL (single-use, 7-day TTL).
- Rate limiting on auth + AI endpoints.

### NFR-4 — Observability
- Sentry on web + iOS + Django + Celery worker.
- Structured logs with correlation IDs (request id, workspace id, user id).
- Per-AI-call telemetry: prompt name, model, tokens, cost, latency.
- Run inspector for every workflow run with full step trace.
...
### NFR-6 — Cost guardrails
- Per-workspace daily AI token budget; over-budget → enqueue + admin notification.
- Provider fallback (Claude → OpenAI/Gemini) on outage or cost spike.
- Anthropic prompt caching for system prompts and large reference docs.
- Embeddings written async, batched, deduped by content hash.

### NFR-7 — Reliability
- Worker restart preserves all in-flight workflow runs.
- Idempotency keys on every workflow side effect.
- Backups: managed Postgres point-in-time recovery; nightly logical dump to object storage.
- Beat wake-up sweep tolerates 60s jitter for delays/waits in MVP.

### NFR-8 — Privacy
- All data is workspace-scoped. No cross-workspace leakage.
- AI prompts never leak data from one workspace into another's context.
- Right-to-delete: workspace owner can request a hard delete that cascades within 30 days.
...
### POST-1 — Stripe / billing (PLAN §14)
- Subscription plans, seat-based pricing, per-workspace metering for AI tokens, free trial, usage caps, dunning.

### POST-2 — Long-tail integrations
- HubSpot full bidirectional sync, Salesforce, Linear, Notion, Microsoft 365 (Outlook + Teams + OneDrive), Zoom, Twilio (SMS/calls), DocuSign, Stripe (as a CRM data source). Pattern: each is its own connector + workflow trigger/action set.

### POST-3 — Multi-language i18n (web + iOS)
- Localization framework, translation pipeline, RTL support evaluation.

### POST-4 — Native Android app
- Kotlin Multiplatform vs native Android Compose decision deferred.

### POST-5 — Enterprise (SSO/SCIM, audit retention, custom roles, sandbox)
