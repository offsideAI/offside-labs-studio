...
- `[✅]` **M9.S4 — As an admin, I can install a pre-built workflow agent from the Agents Marketplace with one click.** *Demo-critical (FR-26) ✅ fully shipped. 15-agent catalog spanning the full ecommerce lifecycle across 9 categories. Backend + frontend (grid + detail with read-only canvas preview + one-click install + sidebar repositioning above CRM records) + demo workspace seed (`tools/seeds/demo_workspace.py`) + presenter script (`DEMO.md`). Hero agent: **Ecommerce Conversion Funnel Optimizer** — a 10-node manual-triggered workflow that one-click-deploys marketing campaign launch → AEO seed → ad sync → landing pages → email welcome → cart recovery → payments → deployment-summary note. Implementation breakdown below.*
...
- `[✅]` **Agents Marketplace v1** (M9.S4, FR-26) — `apps/marketplace` Django app with:
...
### Revision 14 — 2026-05-13 — M9.S4 fully shipped + ecommerce catalog + hero agent + DEMO.md

**M9.S4 flips to ✅.** Five concurrent slices landed:
...
- **Sidebar repositioning.** Primary nav now leads with **Agent Marketplace (★★)** + **Agent Design Studio (★)** above Contacts / Companies / Deals, per the OffsideStudio — Agent Marketplace positioning. The "Automations" sidebar entry renamed to "Agent Design Studio" while keeping the `/automations` route. Cmd-K palette gains "Agent Marketplace" + renamed "Agent Design Studio" items.
...
- **Ecommerce catalog expansion.** Added 4 new `MarketplaceAgentCategory` values (`cart_recovery`, `fulfillment`, `payments`, `customer_service`) via migration `0002_alter_marketplaceagent_category.py` (metadata-only — Django enforces choices at the model layer, not the DB). Added 10 new ecommerce agents to `tools/seeds/marketplace.py` spanning the full lifecycle: Lead from form (📋), Welcome series day 0 (📧), Abandoned cart recovery (🛒), Order received (📦), Shipping dispatched (🚚), Payment success (💳), Failed payment retry (💸), Refund request handler (↩️), Post-purchase review request (⭐), Repeat-purchase nudge (🔁). All use only currently-shipped trigger types and registered actions; all reference `{{ trigger.* }}` for trigger-payload fields and `{{ n<N>.field }}` for step-output chaining. Frontend CATEGORY_LABELS + CATEGORIES filter list updated and reordered to lead with the ecommerce-lifecycle stages (lead → conversion → fulfillment → payments → service) before the foundational categories.
- **Hero agent — Ecommerce Conversion Funnel Optimizer 🚀.** A 15th marketplace agent added at the top of the seed list, designed as the demo opener. **10-node** manual-triggered graph: (n1) init log → (n2) **Launch marketing campaign** across email/social/search/display → (n3) AEO content seed → (n4) ad-platform sync → (n5) landing-page generation → (n6) create Demo Funnel company → (n7) email welcome task → (n8) cart-recovery task → (n9) payment confirmation task → (n10) deployment-summary markdown note. Step chaining uses `{{ n2.status_code }}` → AEO, `{{ n3.status_code }}` → ads, `{{ n6.company_id }}` → all downstream tasks/notes. Description and long_description lead with the marketing-campaign-launch step and use the *"Every lead pulled in becomes a converted paying customer"* phrasing verbatim.
- **Demo seed data.** `tools/seeds/demo_workspace.py` creates an idempotent demo workspace (`acme-demo`) with owner user (`demo@offside.ai` / `DemoOnly123!`), default 6-stage sales pipeline, 8 companies, 20 contacts spread across the companies with realistic titles + lifecycle stages, 12 deals distributed across all pipeline stages with staggered close dates, 5 tasks (mix of open + done), 3 markdown notes. Marketplace agents intentionally NOT pre-installed — the demo opener is *"install your first agent live."*
- **`DEMO.md` presenter script** (new top-level file). 5-minute narrative arc: (0:00) frame the product → (0:25) install the hero agent → (1:30) click Run live → (3:00) switch to Companies to see the records the agent just created → (4:00) bonus Describe-in-English → (5:00) close. Includes pre-stage hash check (curl + manage.py shell one-liners), common-issues table with 9 failure modes + quick fixes, time-pacing tips, and a condensed narrative-arc cheat sheet at the bottom.
...
**FR-26 marked ✅** in PRD v3.2 (revision-history bumped). The catalog ships with **15 agents** spanning **9 categories**. Demo headline is the hero agent.

**Remaining for the conference demo:**
1. **Smoke pass on a real machine** — start the stack, run both seed scripts, walk the 5-min DEMO.md narrative, fix anything that surfaces.
2. **Commit + push** ~19 slices' worth of pending local work.
3. **Deploy to production URLs** — Django → DO App Platform; web → Vercel.
